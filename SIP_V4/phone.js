...
let wallpaperLight = getDbItem("wallpaperLight", "wallpaper.light.webp");  
let wallpaperDark = getDbItem("wallpaperDark", "wallpaper.dark.webp");     
...
let VoiceMailSubscribe = (getDbItem("VoiceMailSubscribe", "1") == "1");                
...
let userAgentStr = getDbItem("UserAgentStr", "Browser Phone "+ appversion +" (SIPJS - "+ sipjsversion +") "+ navUserAgent);   
...
let UiMaxWidth = parseInt(getDbItem("UiMaxWidth", 1273));                                   
...
let MaxDidLength = parseInt(getDbItem("MaxDidLength", 16));          
...
let EnableTextMessaging = (getDbItem("EnableTextMessaging", "1") == "1");               
...
let EnableTextExpressions = (getDbItem("EnableTextExpressions", "1") == "1");           
let EnableTextDictate = (getDbItem("EnableTextDictate", "1") == "1");                   
...
let ChatEngine = getDbItem("ChatEngine", "SIMPLE");    
let XmppServer = getDbItem("XmppServer", "");                
let XmppWebsocketPort = getDbItem("XmppWebsocketPort", "");  
let XmppWebsocketPath = getDbItem("XmppWebsocketPath", "");  
let XmppDomain = getDbItem("XmppDomain", "");                
let profileUser = getDbItem("profileUser", null);            
let XmppRealm = getDbItem("XmppRealm", "");                    
let XmppRealmSeparator = getDbItem("XmppRealmSeparator", "-"); 
...
leftSection.attr("id", "leftContent");
    ...
    phone.css("max-width", "100%");

    // Create the 3-column layout
    var columnContainer = $("<div/>").addClass("column-container");

    // Middle Column (Buddies/Contacts - leftContent)
    var middleColumn = $("<div/>").addClass("history-column");
    middleColumn.attr("id", "leftContent");
    ...
    // Profile User
    // Profile header – KaizenVoiz styled
    var savedQueueId = localDB.getItem("QueueID");
    leftHTML += `<div class="profileContainer" style="padding:10px 12px 8px; display:flex; align-items:center; gap:10px; background:var(--bg-elevated); border-bottom:1px solid var(--border);">`;
    leftHTML += `<div id=UserProfilePic class=buddyIcon style="width:36px; height:36px; border-radius:50%; flex-shrink:0; background-size:cover; background-position:center; background-color:var(--blue-dim); border:2px solid var(--border-strong);"></div>`;
    leftHTML += `<div style="flex:1; min-width:0;">`;
    leftHTML += `<div class=contactNameText style="display:flex; align-items:center; gap:5px; margin:0; font-size:13px; font-weight:700; color:var(--text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">`;
    ...
    leftHTML += `</div>`;
    leftHTML += `<div class=presenceText style="font-size:11px; color:var(--text-muted); margin-top:1px;margin-left: 1px;"><span id=regStatus>&nbsp;</span> <span id=dndStatus></span></div>`;
    leftHTML += `</div>`;

    // Queue + break toggle pill
    if(SipUsername && !window.IsProfilePage){
        leftHTML += `<div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">`;
        if(savedQueueId){
            leftHTML += `<span style="font-size:10px; font-weight:700; font-family:var(--font-mono,'Consolas',monospace); color:var(--text-muted); background:var(--bg-hover); border:1px solid var(--border); border-radius:5px; padding:2px 7px;" title="Queue ID">${savedQueueId}</span>`;
        }
        leftHTML += `<label class="break-switch" title="Toggle break (green=active, red=on break)" style="flex-shrink:0;">
    ...
    // Hidden voicemail notifyer (keep functional)
    leftHTML += `<span id=TxtVoiceMessages class=voiceMessageNotifyer style="display:none;">0</span>`;

    if(false){ leftHTML += "<button id=BtnCreateGroup style='display:none'></button>"; } // TODO

    leftHTML += `</div>`; // profileContainer

    leftHTML += "</td></tr>";
    leftHTML += `<tr id=searchArea><td class=streamSection style="height:42px; box-sizing:border-box; padding:5px 10px;">`;
    leftHTML += `<div style="display:flex; align-items:center; gap:6px; background:var(--input-bg); border:1px solid var(--input-border); border-radius:7px; padding:0 10px; height:30px;">`;
    leftHTML += `<i class="fa fa-search" style="color:var(--text-muted); font-size:11px; flex-shrink:0;"></i>`;
    leftHTML += `<input id=txtFindBuddy type=text autocomplete=none style="flex:1; border:none; background:transparent; outline:none; color:var(--text-primary); font-size:12px; height:100%;">`;
    leftHTML += `</div>`;
    leftHTML += `</td></tr>`;
    ...
    setTimeout(function () {
        ...
        const savedState = localStorage.getItem("isOnBreak");

        if (savedState === "true") {
            breakToggle.checked = false; // On Break
        } else {
            breakToggle.checked = true;  // Active
        }
    ...
    // Right Column (Call Details/Active Calls)
    var rightColumn = $("<div/>").addClass("details-column");
    rightColumn.attr("id", "rightContent");
    rightColumn.attr("class", "details-column rightContent");

    // Conditionally add dial column and right column only if logged in
    if (profileUserID && profileUserID !== "null" && profileUserID !== null && profileUserID !== "undefined") {
        var dialColumn = $("<div/>").addClass("dial-column");
        dialColumn.attr("id", "dialPadSection");
        columnContainer.append(dialColumn);
        columnContainer.append(middleColumn);
    ...
    // Add container to phone
    phone.append(columnContainer);
    ...
    if(SipUsername) $("#UserCallID").html(profilePrepend +""+ SipUsername);
    ...
    // Show dial pad in the left column
    ShowDial();
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    agentSelect.id = "agentId";

    // Add options
    const defaultOpt = new Option("-- Select Agent --", "");
    agentSelect.add(defaultOpt);
    ["6001", "6002", "6003"].forEach(id => {
        agentSelect.add(new Option(id, id));
    });

    // Insert dropdown just before the button
    checkBtn.parentNode.insertBefore(agentSelect, checkBtn);

    // (Optional) save selection
    agentSelect.addEventListener("change", () => {
        const selected = agentSelect.value;
        if (selected) {
            localStorage.setItem("selectedAgentId", selected);
        } else {
...
$("body").append(timerBox).append(checkBtn);

// === State from localStorage ===
let isCheckedIn = localStorage.getItem("isCheckedIn") === "true";
let checkInStartTime = localStorage.getItem("checkInStartTime");
let timerInterval = null;

// === Timer Functions ===
function startTimer() {
    if (!checkInStartTime || timerInterval) return;

    const previousSeconds = parseInt(localStorage.getItem("elapsedBefore") || "0", 10);
    const startTime = new Date(checkInStartTime).getTime();

    timerInterval = setInterval(() => {
        const now = Date.now();
        const currentSession = Math.floor((now - startTime) / 1000);
        const total = previousSeconds + currentSession;

        const hours = Math.floor(total / 3600).toString().padStart(2, "0");
        const minutes = Math.floor((total % 3600) / 60).toString().padStart(2, "0");
        const seconds = (total % 60).toString().padStart(2, "0");

        $("#checkTimer").text(`${hours}:${minutes}:${seconds}`);
    }, 1000);
}
...
function stopTimer() {
    ...
    if (checkInStartTime) {
        const startTime = new Date(checkInStartTime).getTime();
        const now = Date.now();
        currentSession = Math.floor((now - startTime) / 1000);
    }
...
function updateUI() {
    if (isCheckedIn) {
        $("#checkBtn")
            .text("Check Out")
            .css("background-color", "#a71919"); // Red
        startTimer();
    } else {
        $("#checkBtn")
            .text("Check In")
            .css("background-color", "#276f28"); // Green
        stopTimer(); // Display frozen time
    }
}
// === Daily Reset Logic ===
...
if (lastActiveDate !== todayStr) {
    ...
    localStorage.removeItem("elapsedBefore");
...
// Update the active date to today (fresh or not)
localStorage.setItem("lastActiveDate", todayStr);

// === Initial Setup ===
updateUI();

// Show frozen time if not checked in
if (!isCheckedIn) {
    const total = parseInt(localStorage.getItem("elapsedBefore") || "0", 10);
    const hours = Math.floor(total / 3600).toString().padStart(2, "0");
    const minutes = Math.floor((total % 3600) / 60).toString().padStart(2, "0");
    const seconds = (total % 60).toString().padStart(2, "0");
    $("#checkTimer").text(`${hours}:${minutes}:${seconds}`);
...
if (!isCheckedIn) {
    ...
    fetch("https://192.168.5.60/SIPApi/api/v1/ivr/CheckIn", {
        method: "POST",
        body: formData
    })
    .then(res => {
    ...
    .then(() => {
        const now = new Date(); // ✅ local only for UI

        const lastCheckOut = localStorage.getItem("lastCheckOutTime");
        let elapsedBefore = parseInt(localStorage.getItem("elapsedBefore") || "0", 10);

        if (lastCheckOut) {
            const idleSeconds =
                Math.floor((now.getTime() - new Date(lastCheckOut).getTime()) / 1000);
            elapsedBefore += idleSeconds;
        }
        ...
        localStorage.setItem("checkInStartTime", now.toISOString());
        localStorage.setItem("elapsedBefore", elapsedBefore.toString());
    ...
    .catch(err => {
        ...
        fetch("https://192.168.5.60/SIPApi/api/v1/ivr/CheckOut", {
            method: "POST",
            body: formData
        })
        .then(res => {
        ...
        .then(() => {
            // ✅ Save elapsedBefore before resetting
            const startTime = new Date(checkInStartTime).getTime();
            const sessionSeconds = Math.floor((new Date(now).getTime() - startTime) / 1000);

            const elapsedBefore = parseInt(localStorage.getItem("elapsedBefore") || "0", 10) + sessionSeconds;
            localStorage.setItem("elapsedBefore", elapsedBefore.toString());
...
function ApplyThemeColor(){
    ...
    // ── Sync with KaizenVoiz kv-theme (shared across attendance, call-history, phone) ──
    var kvTheme = localStorage.getItem("kv-theme");
    if(kvTheme === "dark" || kvTheme === "light"){
        isDarkTheme = (kvTheme === "dark");
        // Keep UiThemeStyle in sync so internal logic matches
        UiThemeStyle = kvTheme;
    } else {
        // Fall back to UiThemeStyle setting
        ...
        // Write resolved value back to kv-theme so other pages stay in sync
        localStorage.setItem("kv-theme", isDarkTheme ? "dark" : "light");
    }
    ...
    // ── Apply data-theme attribute (used by kaizenvoiz-theme.css variables) ──
    document.documentElement.setAttribute("data-theme", isDarkTheme ? "dark" : "light");

    // ── Load phone colour stylesheet (phone.dark.css / phone.light.css) ──
    var cssUrl = hostingPrefix + (isDarkTheme ? "phone.dark.css" : "phone.light.css");
    if($("#colorSchemeMode").length == 0){
        $("head").append('<link rel="stylesheet" id="colorSchemeMode" />');
    }
    $("#colorSchemeMode").attr("href", cssUrl);

    // ── Wallpaper ──
    if($("#colorSchemeModeSheet").length == 0){
    ...
    // ── Keep kaizenvoiz-theme.css patch in sync if loaded ──
    // Dispatch a custom event so any listening header toggle button can update its icon state
    document.dispatchEvent(new CustomEvent("kv-theme-changed", { detail: { theme: isDarkTheme ? "dark" : "light" } }));
}
...
function decodePAssertedIdentity(rawUri) {
    // Extract only the hex portion before ":"
    var hexMatch = rawUri.trim().split(":")[0].trim();

    // Validate it's hex only (optional, but safe)
    if (/^[0-9a-fA-F]+$/.test(hexMatch)) {
        try {
            let hexToString = hexMatch.match(/.{1,2}/g).map(byte => String.fromCharCode(parseInt(byte, 16))).join('');
            return hexToString;
        } catch (e) {
        ...
        console.warn("Input is not pure hex:", hexMatch);
        return null;
    }
}
function ReceiveCall(session) {
    ...
    else if (session.terminate) session.terminate();
    console.log("Call rejected: agent is paused (break or not checked in)");
    return;
}
    ...
    if (sipHeaders.hasOwnProperty("P-Asserted-Identity")) {
        ...
        // Decode hex PAI
        decodedAssertIdData = decodePAssertedIdentity(rawUri);
        console.log("Decoded P-Asserted-Identity:", decodedAssertIdData);

        // Store full decoded value
        session.data = session.data || {};
        session.data.decodedAssertId = decodedAssertIdData;

        // Extract Channel_ID and store for later API
        const parts = decodedAssertIdData.split("|");
        if (parts.length > 0) {
            let channelId = parts[parts.length - 1];

            session.channelId = channelId;
            session.decodedAssertId = decodedAssertIdData;

            console.log("Decoded Assert ID:", decodedAssertIdData);
            console.log("Extracted Channel_ID:", channelId);
        }

        // Optionally update DID if trust domain matches
        if (rawUri.includes("<sip:")) {
            ...
            if (uriParts[1].endsWith("@" + SipDomain)) {
                ...
                did = assertId.user;
                console.log("Found trusted P-Asserted-Identity, new DID:", did);
            } else {
                console.warn("PAI not in trust domain:", rawUri);
            }
        }
    }
    ...
    if (buddyObj == null) {
        ...
        if (sipHeaders.hasOwnProperty("X-Buddytype")) {
            var val = sipHeaders["X-Buddytype"][0].raw;
            if (["contact", "extension", "xmpp", "group"].includes(val)) buddyType = val;
        }
    ...
    lineObj.SipSession.data.decodedAssertId = decodedAssertIdData;
    lineObj.SipSession.data.channelId = session.data.channelId || null;
    ...
    const agentId = getDbItem("SipUsername", "");
    const timestamp = new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 17);
    const recordId = `0001${agentId}${timestamp}`;

    // Store session data
    lineObj.SipSession.data.agentId = agentId;
    lineObj.SipSession.data.msiDn = did;
    lineObj.SipSession.data.recordId = recordId;
   ...
   if (session.channelId) {
    ...
    // fetch("https://192.168.5.57:8181/restapi/rest/api/v4/logging/t1/agentinfo", {
    fetch("https://192.168.5.60/SIPApi/api/v1/ivr/InsertAgentStatus", {
    ...
    .then(res => {
        if (!res.ok) throw new Error("AgentInfo API call failed: " + res.status);
        return res.text();
    })
    .then(result => console.log("Agent info logged successfully:", result))
    ...
    if (EnableVideoCalling && lineObj.SipSession.request.body &&
        lineObj.SipSession.request.body.indexOf("m=video") > -1) {
        lineObj.SipSession.data.withvideo = true;
    }

    // Delegates
    lineObj.SipSession.delegate = {
        onBye: sip => onSessionReceivedBye(lineObj, sip),
    ...
    lineObj.SipSession.incomingInviteRequest.delegate = {
        onCancel: sip => onInviteCancel(lineObj, sip)
    };

    // Check-in & DND handling
    if (!isCheckedIn) {
        console.log("Agent is checked out, rejecting call.");
        lineObj.SipSession.data.earlyReject = true;
        RejectCall(lineObj.LineNumber, true);
        return;
    }
    //  if (isOnBreak) {
    //     console.log("Agent is on break, rejecting call.");
    //     var lineObj = { SipSession: session, LineNumber: newLineNumber + 1 };
    //     session.data = session.data || {};
    ...
    // }
    if (DoNotDisturbEnabled || DoNotDisturbPolicy == "enabled") {
        if (!(DoNotDisturbEnabled && buddyObj.EnableDuringDnd)) {
    ...
    if (CurrentCalls >= 1 && (!CallWaitingEnabled || CallWaitingEnabled == "disabled")) {
    ...
    $("#line-" + lineObj.LineNumber + "-msg").html(lang.incoming_call).show();
...
function toQueueAgent(agentId) {
   
    if (!agentId) return "";
    if (agentId.startsWith("pjsip/")) {
        return agentId;
    }
    if (agentId.startsWith("webrtc_")) {
        return "pjsip/" + agentId.replace("webrtc_", "");
    }
    return "pjsip/" + agentId;
}
function pauseAgentQueue(agentId) {
    const queueAgent = toQueueAgent(agentId);

    console.log("Pausing queue for:", queueAgent);

    const formData = new FormData();
    formData.append("agentno", queueAgent);

    return fetch("https://192.168.5.57:8181/app/rest/api/v2/queue/pause", {
        method: "POST",
        body: formData
    })
    .then(res => res.text())
...
function resumeAgentQueue(agentId) {
    const queueAgent = toQueueAgent(agentId);

    console.log("Resuming queue for:", queueAgent);

    const formData = new FormData();
    formData.append("agentno", queueAgent);

    return fetch("https://192.168.5.57:8181/app/rest/api/v2/queue/unpause", {
        method: "POST",
        body: formData
    })
    .then(res => res.text())
...
function AnswerAudioCall(lineNumber) {
    ...
    if(HasAudioDevice == false){
        ...
        $("#line-" + lineNumber + "-crmForm").addClass("active");
    // Update UI
    $("#line-" + lineObj.LineNumber + "-AnswerCall").hide();
    ...
    // 🚀 OPEN CRM FORM AUTOMATICALLY
    setTimeout(() => {
        openCrmWindow(lineNumber);
    }, 300);
...
function onInviteCancel(lineObj, response) {
    ...
    if (reason !== undefined && reason.length > 0) {
        for (var i = 0; i < reason.length; i++) {
            ...
            if (items.length >= 2 &&
                ...
                reasonText = "Call completed elsewhere";
                break;
            }
        }
    }
    ...
    // Log info
    console.log("Call cancelled:", reasonText);
...
UpdateAgentCallStatus(1); // Agent is now on a call
    if(includeVideo){
...
function endSession(lineNum) {
    ...
    if (!lineObj || !lineObj.SipSession) return;

    

    // ⭐ STORE CRM DATA BEFORE SESSION ENDS
    window.crmCache[lineNum] = {
        recordId: lineObj.SipSession.data.recordId,
        decodedAssertId: lineObj.SipSession.data.decodedAssertId,
        displayNumber: lineObj.DisplayNumber,
        callstart: lineObj.SipSession.data.callstart
    };

    console.log("CRM data cached for line:", lineNum, window.crmCache[lineNum]);
...
function addToCallHistory(call) {
    const callInfo = {
        id: Date.now(),
        number: call.number || call.DisplayNumber,
...
function updateCallHistoryDisplay() {
    const historyList = $("#callHistoryList");
    historyList.empty();

    if (callHistory.length === 0) {
        historyList.append("<div class='no-calls'>No calls to display</div>");
        return;
    }

    callHistory.forEach(call => {
        const callItem = $("<div/>")
            .addClass("call-history-item")
            .attr("data-call-id", call.id)
            .html(`
                <div><strong>${call.name || call.number}</strong></div>
                <div>${call.type} · ${call.time}</div>
            `);

        callItem.click(() => showCallDetails(call));
        historyList.append(callItem);
    });
}
function showCallDetails(call) {
    const detailsDiv = $("#callDetails");
    $(".call-history-item").removeClass("selected");
    $(`.call-history-item[data-call-id='${call.id}']`).addClass("selected");

    detailsDiv.html(`
        <div class='call-info'>
            <div><span class='call-info-label'>Name:</span> ${call.name || 'Unknown'}</div>
            <div><span class='call-info-label'>Number:</span> ${call.number}</div>
            <div><span class='call-info-label'>Time:</span> ${call.time}</div>
            <div><span class='call-info-label'>Duration:</span> ${formatDuration(call.duration)}</div>
            <div><span class='call-info-label'>Type:</span> ${call.type}</div>
            <div><span class='call-info-label'>Status:</span> ${call.status}</div>
        </div>
        <div style="text-align: center; margin-top: 20px;">
            <button class="dialButtons dialButtonsDial" onclick="DialByLine('audio', null, '${call.number}')">
...
function saveCallHistory() {
    localDB.setItem("CallHistory", JSON.stringify(callHistory));
}
function loadCallHistory() {
    const saved = localDB.getItem("CallHistory");
    if (saved) {
...
// Phone Lines
var Line = function(lineNumber, displayName, displayNumber, buddyObj,userToUser){
...
function ShowDial(){
    var dialPadSection = $("#dialPadSection");
    dialPadSection.empty();

    var html = `
    <div style="padding:16px 12px 0; text-align:center;">
      <div style="font-size:11px; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; color:var(--text-muted,#888); margin-bottom:14px;">
        <i class="fa fa-phone" style="margin-right:5px; color:var(--accent,#2f83ff);"></i> Dial Pad
      </div>

      <div style="display:flex; align-items:center; gap:6px; margin-bottom:16px; background:var(--input-bg,#1e1f28); border:1px solid var(--input-border,rgba(255,255,255,0.12)); border-radius:8px; padding:4px 10px;">
        <input id=dialText class=dialTextInput
          oninput="handleDialInput(this, event)"
          onkeydown="dialOnkeydown(event, this)"
          style="flex:1; height:36px; font-size:20px; font-family:var(--font-mono,'Consolas',monospace); font-weight:600; letter-spacing:0.08em; background:transparent; border:none; outline:none; color:var(--text-primary,#e8e9f0);"
          placeholder="">
        <button id=dialDeleteKey class=roundButtons onclick="KeyPress('del')"
          style="font-size:15px; width:30px; height:30px; display:none; flex-shrink:0; color:var(--text-muted,#888); background:none; border:none; cursor:pointer;">⌫</button>
      </div>
    ...
    keys.forEach(function(item){
        html += `<button class=dialButtons onclick="KeyPress('${item.k}')"
          style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:52px; border-radius:8px; border:1px solid var(--border,rgba(255,255,255,0.08)); background:var(--bg-elevated,#2a2b35); cursor:pointer; transition:background 0.15s;">
          <span style="font-size:18px; font-weight:600; line-height:1; color:var(--text-primary,#e8e9f0);">${item.k}</span>
          <span style="font-size:9px; color:var(--text-muted,#888); letter-spacing:0.05em; margin-top:2px;">${item.s}</span>
        </button>`;
    });
    ...
    html += `</div>
      ...
      <div style="display:flex; justify-content:center; padding-bottom:16px;">
        <button class="dialButtonsDial" id=dialAudio title="${lang.audio_call}" onclick="DialByLine('audio')"
          style="width:56px; height:56px; border-radius:50%; border:none; cursor:pointer; font-size:20px; background:var(--green,#3dbe6c); color:#fff; box-shadow:0 4px 14px rgba(61,190,108,0.35); transition:background 0.15s, transform 0.1s; display:flex; align-items:center; justify-content:center;">
          <i class="fa fa-phone"></i>
        </button>
      </div>
    </div>`;

    dialPadSection.html(html);
    $("#dialText").focus();
}
function handleDialInput(obj, event){
    ...
    if($("#dialText").val().length > 0){
        $("#dialDeleteKey").css("display","flex");
    } else {
        $("#dialDeleteKey").hide();
    }
}
...
function DialByLine(type, buddy, numToDial, CallerID, extraHeaders){
    ...
    const callInfo = {
        number: numToDial,
        DisplayName: CallerID || "Outbound Call",
        type: "outbound",
        status: "initiated",
        duration: 0
    };
...
function AddLineHtml(lineObj, direction){
    ...
    // html += "<div class=callingusertouser>"+ lineObj.UsertoUser +"</div>";

   if (lineObj.SipSession && lineObj.SipSession.data && lineObj.SipSession.data.decodedAssertId) {
    const parts = lineObj.SipSession.data.decodedAssertId.split("|");

    html += "<div class='calling-assertid'>";
    ...
    // === CRM Sliding Sidebar Form ===
// html += "<div id='line-" + lineObj.LineNumber + "-crmForm' class='crmForm'>";

// Close button
// html += "  <button class='crmCloseBtn' onclick=\"$('#line-" + lineObj.LineNumber + "-crmForm').removeClass('active')\">&times;</button>";

// html += "  <h3 style='margin-top:0;'>Call Details</h3>";

// // If Assert ID is available, split and display fields
// if (lineObj.SipSession && lineObj.SipSession.data && lineObj.SipSession.data.decodedAssertId) {
//     const parts = lineObj.SipSession.data.decodedAssertId.split("|");

//     // Expected parts: [recordId, uniqueId, clientId]
//     if (parts.length >= 1) {
//         html += "  <div><label>Unique ID:</label>" +
//             "<input type='text' value='" + parts[0] + "' readonly " +
//             "style='width:100%; margin-bottom:8px;color: #ffff; padding:5px; border:1px solid #ccc; border-radius:5px;'/></div>";
//     }
//     if (parts.length >= 2) {
//         html += "  <div><label>Record ID:</label>" +
//             "<input type='text' value='" + parts[1] + "' readonly " +
//             "style='width:100%; margin-bottom:8px; color: #ffff;padding:5px; border:1px solid #ccc; border-radius:5px;'/></div>";
//     }
//     if (parts.length >= 3) {
//         html += "  <div><label>Client ID:</label>" +
//             "<input type='text' value='" + lineObj.DisplayNumber + "' readonly " +
//             "style='width:100%; margin-bottom:8px; color: #ffff;padding:5px; border:1px solid #ccc; border-radius:5px;'/></div>";
//     }
// }

// // Description box
// html += "  <div><label>Description:</label>" +
//     "<textarea id='line-" + lineObj.LineNumber + "-desc' " +
//     "style='width:100%; height:70px; margin-top:5px; padding:5px; border:1px solid #ccc; border-radius:5px;'></textarea></div>";

// // Submit button
// html += "  <button onclick=\"submitCrmForm('" + lineObj.LineNumber + "', false)\" " +
//     "style='margin-top:12px; width:100%; padding:8px; background:#007bff; color:#fff; border:none; border-radius:6px; cursor:pointer;'>Submit</button>";
...
function ShowMyProfile() {
    ...
    $("#BtnFreeDial, #SettingsMenu, #logout, #reglink, #UserCallID, #UserProfilePic").hide();
    $("#myContacts, #searchArea").hide();
    $("#actionArea").empty();
    ...
    if (EnableAccountSettings == true) {
        html += "<div class='UiTextHeadingL'><i class=\"fa fa-user-circle-o UiTextHeadingIcon\" style=\"background-color:#a93a3a\"></i> " + lang.account + "</div>";
    }

    html += "<div id='Configure_Extension_Html' style='display:block'>";

    html += "<div class='UiTextL'>" + lang.sip_username + ":</div>";
    html += "<div><input id='Configure_Account_SipUsername' class='UiInputTextL' type='text' placeholder='" + lang.eg_sip_username + "' value='" + getDbItem("SipUsername", "") + "'></div>";

    html += "<div class='UiTextL'>" + lang.sip_password + ":</div>";
    html += "<div><input id='Configure_Account_SipPassword' class='UiInputTextL' type='password' placeholder='" + lang.eg_sip_password + "' value='" + getDbItem("SipPassword", "") + "'></div>";

    html += "</div>"; // end Configure_Extension_Html
    html += "</div>"; // end UiSideField
    ...
    buttons.push({
        ...
        action: function () {
            ...
            if (EnableAccountSettings) {
                if ($("#Configure_Account_SipUsername").val().trim() == "" ||
                    $("#Configure_Account_SipPassword").val().trim() == "") {
                    Alert("Validation", "SIP Credentials Required");
                    return;
                }
            }
            ...
            var sipValues = {
                wssServer: "192.168.5.61",
                WebSocketPort: "8089",
                ServerPath: "/wS",
                SipDomain: "192.168.5.61",
                SipUsername: $("#Configure_Account_SipUsername").val().trim(),
                SipPassword: $("#Configure_Account_SipPassword").val().trim()
            };

            let agentId = sipValues.SipUsername;
            let agentName = "";
            let queueId = "";

            // ✅ Step 1: ACD User Validation
            // fetch("https://192.168.5.57:8181/restapi/rest/api/acdqueue/" + encodeURIComponent(agentId))
            fetch("https://192.168.5.60/SIPApi/api/v1/ivr/acdqueue/" + encodeURIComponent(agentId))
                .then(res => res.text())
                .then(text => {
                    console.log("ACD Response:", text);

                    if (text.includes("Agent not registered")) {
                        Alert("Invalid User", "Agent not registered");
                        throw "Stop here";
                    }

                    let agentNameMatch = text.match(/Agent Name:\s*([^,]+)/i);
                    let queueMatch = text.match(/Queue ID:\s*([A-Za-z0-9]+)/i);


                    if (agentNameMatch && queueMatch) {
                        agentName = agentNameMatch[1].trim();
                        queueId = queueMatch[1].trim();

                        localDB.setItem("AgentName", agentName);
                        localDB.setItem("QueueID", queueId);

                        console.log("Agent Verified:", agentName, queueId);
                    }

                    // Step 2: Check SIP Status
                    // return fetch("https://192.168.5.57:8181/restapi/rest/api/sipstatus?SipUsername=" + encodeURIComponent(agentId));
                    return fetch("https://192.168.5.60/SIPApi/api/v1/ivr/sipstatus?SipUsername=" + encodeURIComponent(agentId));

                })
                .then(res => res.text())
                .then(text => {
                    let status = text.trim().toUpperCase();
                    ...
                    else if (status === "LOGGED_OUT" || status === "NOT_FOUND") {
                        console.log("SIP can be inserted. Status:", status);
                        saveSipData(agentName, queueId);
                    }
                    else {
                        Alert("Error", "Unexpected SIP Status: " + status);
                        throw "Stop here";                       
                    }
            ...
            function saveSipData(agentName, queueId) {
                localDB.setItem("wssServer", sipValues.wssServer);
                localDB.setItem("WebSocketPort", sipValues.WebSocketPort);
                localDB.setItem("ServerPath", sipValues.ServerPath);
                localDB.setItem("profileName", agentName);
                localDB.setItem("SipDomain", sipValues.SipDomain);
                localDB.setItem("SipUsername", sipValues.SipUsername);
                localDB.setItem("SipPassword", sipValues.SipPassword);
                localDB.setItem("QueueID", queueId);

                const formData = new URLSearchParams({
                    UniqueID: localDB.getItem("profileUserID"),
                    WssServer: sipValues.wssServer,
                    WebSocketPort: sipValues.WebSocketPort,
                    ServerPath: sipValues.ServerPath,
                    ProfileName: agentName,
                    SipDomain: sipValues.SipDomain,
                    SipUsername: sipValues.SipUsername,
                    QUEUE_ID: queueId,
                    SipPassword: sipValues.SipPassword
                });

                let pureAgentId = sipValues.SipUsername.replace(/^webrtc_/i, "").trim();

                const queueform = new URLSearchParams({
                    queue: queueId,
                    agentId: pureAgentId
                });
                // fetch("https://192.168.5.57:8181/restapi/rest/api/v4/logging/t5/insertsip", {

                fetch("https://192.168.5.60/SIPApi/api/v1/ivr/saveSipAccount", {
                    method: "POST",
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    body: formData
                })
                .then(r => r.text())
                .then(r => {
                    console.log("SIP Insert:", r);
                    return fetch("https://192.168.5.57:8181/restapi/rest/api/add", {
                        method: "POST",
                        headers: { "Content-Type": "application/x-www-form-urlencoded" },
                        body: queueform
                    });
    ...
    $.each(buttons, function (i, obj) {
        $("<button>" + obj.text + "</button>").click(obj.action).appendTo("#ButtonBar");
    });
...
function isAgentOnCall() {
    for (let i = 0; i < Lines.length; i++) {
        let lineObj = Lines[i];
        if (lineObj && lineObj.SipSession && lineObj.SipSession.state !== SIP.SessionState.Terminated) {
            return true; // Active call exists
        }
    }
    return false; // No active call
}

function UpdateAgentCallStatus(Status) {
    const sipUsername = localDB.getItem("SipUsername");
    if (!sipUsername) return;

    const formData = new URLSearchParams();
    formData.append("SipUsername", sipUsername);
...
function openCrmWindow(lineNumber) {
    const lineObj = FindLineByNumber(lineNumber);
    if (!lineObj || !lineObj.SipSession || !lineObj.SipSession.data) {
    ...
    window.crmCache[lineNumber] = { uniqueId, recordId, clientId };
    ...
    // Escape values to avoid breaking the template literal
    const safeRecord = (recordId || "").replace(/`/g, "");
    const safeClient = (clientId || "").replace(/`/g, "");
    const safeUnique = (uniqueId || "").replace(/`/g, "");
    const safeCase   = caseNum.replace(/`/g, "");
    const safeLine   = String(lineNumber).replace(/`/g, "");
...
<title>Complaint Form – Agent Tool</title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { background: #060a0f; font-family: 'IBM Plex Sans', sans-serif; padding: 16px; }
  @keyframes slideUp { from { opacity:0; transform:translateY(16px) scale(0.97); } to { opacity:1; transform:none; } }
  .popup {
    background: var(--surface); border: 1px solid var(--border);
    border-radius: 10px; width: 100%; max-width: 640px; margin: 0 auto;
  ...
  .popup-header {
    ...
    background:var(--surface); border-radius:10px 10px 0 0; position:sticky; top:0; z-index:10;
  }
  .header-left { display:flex; align-items:center; gap:10px; }
  .header-icon {
    width:32px; height:32px;
    background:linear-gradient(135deg,#f85149 0%,#bf2828 100%);
    border-radius:6px; display:flex; align-items:center; justify-content:center; font-size:15px;
  }
  .header-title { font-family:'IBM Plex Mono',monospace; font-size:13px; font-weight:600; color:var(--text); letter-spacing:.04em; text-transform:uppercase; }
  .header-subtitle { font-size:11px; color:var(--text-muted); margin-top:2px; font-family:'IBM Plex Mono',monospace; }
  .case-badge {
    font-family:'IBM Plex Mono',monospace; font-size:11px; color:var(--accent-hover);
  ...
  .close-btn {
    ...
    border-radius:5px; font-size:18px; line-height:1; transition:background .15s,color .15s;
  }
  .close-btn:hover { background:var(--tag-bg); color:var(--text); }
  ...
  label { font-size:11px; font-weight:500; color:var(--text-muted); text-transform:uppercase; letter-spacing:.06em; font-family:'IBM Plex Mono',monospace; }
  label .req { color:var(--danger); margin-left:2px; }
  input, select, textarea {
    background:var(--bg); border:1px solid var(--border); border-radius:6px;
    color:var(--text); font-family:'IBM Plex Sans',sans-serif; font-size:13px;
    padding:7px 10px; transition:border-color .15s,box-shadow .15s; outline:none; width:100%;
  }
  ...
  input:focus:not([readonly]), select:focus, textarea:focus {
    border-color:var(--border-active); box-shadow:0 0 0 3px rgba(88,166,255,.1);
  }
  select {
    appearance:none;
  ...
  .priority-btn {
    ...
    background:var(--bg); color:var(--text-muted); font-family:'IBM Plex Mono',monospace;
    font-size:11px; font-weight:600; letter-spacing:.05em; cursor:pointer; text-align:center;
    text-transform:uppercase; transition:all .15s;
  }
  .priority-btn:hover { border-color:var(--text-muted); color:var(--text); }
  .priority-btn.active-low  { border-color:var(--success); background:rgba(63,185,80,.1); color:var(--success); }
  .priority-btn.active-med  { border-color:var(--warning); background:rgba(210,153,34,.1); color:var(--warning); }
  .priority-btn.active-high { border-color:var(--danger);  background:rgba(248,81,73,.1);  color:var(--danger); }
  .tags-container {
    background:var(--bg); border:1px solid var(--border); border-radius:6px;
    padding:6px 8px; display:flex; flex-wrap:wrap; gap:5px;
    min-height:38px; align-items:center; cursor:text; transition:border-color .15s;
  }
  .tags-container:focus-within { border-color:var(--border-active); box-shadow:0 0 0 3px rgba(88,166,255,.1); }
  .tag {
    background:var(--tag-bg); border:1px solid var(--border); color:var(--text-muted);
    font-size:11px; padding:2px 7px; border-radius:4px; display:flex; align-items:center; gap:4px;
    font-family:'IBM Plex Mono',monospace;
  }
  .tag-remove { cursor:pointer; opacity:.6; font-size:13px; line-height:1; }
  ...
  .tag-input {
    ...
    font-size:12px !important; color:var(--text) !important; flex:1; min-width:80px; box-shadow:none !important;
  }
  .info-bar {
    display:flex; align-items:center; gap:8px; padding:10px 12px;
    background:rgba(31,111,235,.08); border:1px solid rgba(31,111,235,.2);
    border-radius:6px; font-size:12px; color:#7eb8f7;
  }
  .callback-toggle {
    display:flex; align-items:center; justify-content:space-between;
  ...
  .callback-toggle:hover { border-color:var(--text-muted); }
  .callback-toggle.active { border-color:#3fb950; background:rgba(63,185,80,.06); }
  .callback-toggle-left { display:flex; align-items:center; gap:10px; }
  .callback-icon {
    width:30px; height:30px; background:rgba(63,185,80,.12);
    border:1px solid rgba(63,185,80,.25); border-radius:6px;
    display:flex; align-items:center; justify-content:center; font-size:14px;
  }
  .callback-toggle-title { font-size:13px; font-weight:500; color:var(--text); }
  .callback-toggle-sub { font-size:11px; color:var(--text-muted); margin-top:1px; font-family:'IBM Plex Mono',monospace; }
  .toggle-switch { position:relative; width:38px; height:21px; flex-shrink:0; }
  .toggle-switch input { opacity:0; width:0; height:0; }
  .toggle-slider { position:absolute; inset:0; background:var(--border); border-radius:21px; transition:background .2s; cursor:pointer; }
  .toggle-slider::before { content:''; position:absolute; width:15px; height:15px; left:3px; top:3px; background:#fff; border-radius:50%; transition:transform .2s; }
  .toggle-switch input:checked + .toggle-slider { background:var(--success); }
  .toggle-switch input:checked + .toggle-slider::before { transform:translateX(17px); }
  .callback-panel {
    display:none; flex-direction:column; gap:12px; padding:14px;
    background:rgba(63,185,80,.04); border:1px solid rgba(63,185,80,.2);
  ...
  .time-slot:hover { border-color:var(--success); color:var(--success); background:rgba(63,185,80,.07); }
  .time-slot.selected { border-color:var(--success); background:rgba(63,185,80,.12); color:var(--success); font-weight:600; }
  .callback-summary {
    display:none; align-items:center; gap:6px; padding:6px 10px;
    background:rgba(63,185,80,.1); border:1px solid rgba(63,185,80,.25);
    border-radius:5px; font-family:'IBM Plex Mono',monospace; font-size:11px; color:var(--success); margin-top:2px;
  }
  .callback-summary.visible { display:flex; }
  .popup-footer {
    display:flex; align-items:center; justify-content:space-between;
    padding:13px 18px; border-top:1px solid var(--border);
    background:var(--surface2); border-radius:0 0 10px 10px; gap:10px;
  }
  .footer-left { font-size:11px; color:var(--text-muted); font-family:'IBM Plex Mono',monospace; }
  .footer-left span { color:var(--danger); }
  .btn-group { display:flex; gap:8px; }
  .btn {
    font-family:'IBM Plex Sans',sans-serif; font-size:13px; font-weight:500;
    padding:8px 16px; border-radius:6px; border:none; cursor:pointer; transition:all .15s; letter-spacing:.01em;
  }
  .btn-ghost { background:transparent; color:var(--text-muted); border:1px solid var(--border); }
  .btn-ghost:hover { background:var(--tag-bg); color:var(--text); border-color:var(--text-muted); }
  .btn-primary { background:linear-gradient(135deg,#1f6feb,#0e5acf); color:#fff; box-shadow:0 1px 3px rgba(0,0,0,.3); }
  .btn-primary:hover { background:linear-gradient(135deg,#388bfd,#1f6feb); transform:translateY(-1px); box-shadow:0 4px 12px rgba(31,111,235,.35); }
  ...
  .success-icon {
    width:56px; height:56px; background:rgba(63,185,80,.12);
    border:1px solid rgba(63,185,80,.3); border-radius:50%;
    display:flex; align-items:center; justify-content:center; font-size:24px;
  }
  .success-title { font-size:15px; font-weight:600; color:var(--text); }
  .success-sub { font-size:12px; color:var(--text-muted); font-family:'IBM Plex Mono',monospace; }
  .popup::-webkit-scrollbar { width:5px; }
  .popup::-webkit-scrollbar-track { background:transparent; }
  .popup::-webkit-scrollbar-thumb { background:var(--border); border-radius:10px; }
...
<div class="popup" id="popup">
  ...
  <div class="popup-header">
    <div class="header-left">
      ...
      <div>
        <div class="header-title">Customer Log</div>
        <div class="header-subtitle">Contact Center · Agent Workspace</div>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:10px;">
      <div class="case-badge" id="caseId">${safeUnique}</div>
    </div>
  </div>
  ...
  <form id="complaintForm" onsubmit="submitForm(event)">
    <div class="popup-body">
      ...
      <div>
        ...
        <div class="row row-3">
          <div class="form-group">
            <label>Record-ID <span class="req">*</span></label>
            <input type="text" id="accountId" value="${safeRecord}" readonly required>
          </div>
          <div class="form-group">
            <label>Phone</label>
            <input type="tel" id="phone" value="${safeClient}" readonly>
          </div>
          <div class="form-group">
            <label>Channel</label>
            <select id="channel">
      ...
      <div>
        ...
        <div class="form-group" style="margin-bottom:10px;">
          ...
          <div class="priority-group">
            <button type="button" class="priority-btn" onclick="setPriority('low',this)">▼ Low</button>
            <button type="button" class="priority-btn active-med" onclick="setPriority('med',this)">● Medium</button>
            <button type="button" class="priority-btn" onclick="setPriority('high',this)">▲ High</button>
          </div>
          <input type="hidden" id="priorityVal" value="med">
        </div>
        <div class="form-group">
          <label>Complaint Summary <span class="req">*</span></label>
          <textarea id="descriptionBox" placeholder="Briefly describe the customer's complaint…" required></textarea>
        </div>
      </div>

      <!-- Resolution -->
      <div>
        <div class="section-label">Resolution &amp; Actions</div>
        <div class="row row-2" style="margin-bottom:10px;">
          <div class="form-group">
            <label>Status</label>
            <select id="status">
        ...
        <div class="form-group" style="margin-bottom:10px;">
          ...
          <textarea id="resolutionNotes" placeholder="Steps taken, commitments made…" style="min-height:65px;"></textarea>
        </div>
      ...
      <!-- Callback Scheduling -->
      <div>
        <div class="section-label">Callback Scheduling</div>
        <div class="callback-toggle" id="callbackToggle" onclick="toggleCallback(event)">
          <div class="callback-toggle-left">
            <div class="callback-icon">📞</div>
            <div>
              <div class="callback-toggle-title">Schedule a Callback</div>
              <div class="callback-toggle-sub">Book a follow-up call with this customer</div>
            </div>
          </div>
          <label class="toggle-switch" onclick="event.stopPropagation()">
            <input type="checkbox" id="callbackCheck" onchange="toggleCallback(event)">
            <span class="toggle-slider"></span>
          </label>
        </div>
        <div class="callback-panel" id="callbackPanel">
          <div class="row row-2">
            <div class="form-group">
              <label>Callback Date <span class="req">*</span></label>
              <input type="date" id="callbackDate" onchange="updateSummary()">
            </div>
            <div class="form-group">
              <label>Callback Time <span class="req">*</span></label>
            <div class="time-spinner-inline">

                <!-- HOUR -->
                <div class="time-box">
                    <input type="text" id="hourInput" value="09" maxlength="2"
                        oninput="validateHour(this)" />
                    <div class="spinner-arrows">
                    <button type="button" onclick="changeHour(1)">▲</button>
                    <button type="button" onclick="changeHour(-1)">▼</button>
                    </div>
                </div>

                <div class="spinner-sep">:</div>

                <!-- MINUTE -->
                <div class="time-box">
                    <input type="text" id="minuteInput" value="00" maxlength="2"
                        oninput="validateMinute(this)" />
                    <div class="spinner-arrows">
                    <button type="button" onclick="changeMinute(1)">▲</button>
                    <button type="button" onclick="changeMinute(-1)">▼</button>
                    </div>
                </div>
          ...
          <div class="row row-2">
            <div class="form-group">
              ...
              <input type="tel" id="callbackPhone" placeholder="+91-7894673982" oninput="updateSummary()">
            </div>
            <div class="form-group">
              <label>Assigned Agent</label>
              <select id="callbackAgent" onchange="updateSummary()">
                <option value="Self">— Self (me) —</option>
                <option>Any Available Agent</option>
                <option>Tier 2 Support</option>
                <option>Supervisor</option>
                <option>Technical Team</option>
              </select>
          ...
          <div class="form-group">
            ...
            <input type="text" id="callbackNotes" placeholder="e.g. Customer available after 2pm…">
          </div>
          <div class="callback-summary" id="callbackSummary">
            <span>📅</span><span id="summaryText">—</span>
          </div>
        </div>
      </div>
    ...
    <div class="popup-footer">
      <div class="footer-left"><span>*</span> Required fields</div>
      <div class="btn-group">
  ...
  <div class="success-state" id="successState">
    ...
    <div class="success-title">Saved Successfully</div>
    <div class="success-sub">${safeUnique}</div>
    <div style="font-size:12px;color:var(--text-muted);max-width:320px;line-height:1.6;">
      The case has been created. A confirmation email will be sent to the customer.
    </div>
    <div class="callback-summary visible" id="successCallback" style="display:none;">
      <span>📞</span><span id="successCallbackText"></span>
    </div>
  </div>
...
function changeMinute(step) {
    ...
    m = (m + step + 60) % 60;
    document.getElementById("minuteInput").value = String(m).padStart(2, '0');
    updateSummary();
}
...
function validateMinute(el) {
  ...
  function setPriority(level, btn) {
    document.querySelectorAll(".priority-btn").forEach(function(b) { b.className = "priority-btn"; });
    btn.classList.add("active-" + level);
    document.getElementById("priorityVal").value = level;
  }
  ...
  function handleTag(e) {
    ...
    if (e.key === "Backspace" && !e.target.value) {
      var tags = document.querySelectorAll("#tagsContainer .tag");
  ...
  function toggleCallback(e) {
    var check  = document.getElementById("callbackCheck");
    var panel  = document.getElementById("callbackPanel");
    var toggle = document.getElementById("callbackToggle");
    if (e && e.target !== check) check.checked = !check.checked;
    if (check.checked) {
      ...
      document.getElementById("callbackSummary").classList.remove("visible");
    }
  }
  ...
  function pickSlot(el, time) {
    ...
    document.getElementById("callbackTime").value = time;
    updateSummary();
  }
  ...
  function updateSummary() {
    ...
    var time = document.getElementById("hourInput").value + ":" +
               document.getElementById("minuteInput").value;

    var phone = document.getElementById("callbackPhone").value;
    var agent = document.getElementById("callbackAgent").value;

    var summary     = document.getElementById("callbackSummary");
  ...
  function saveDraft() {
    ...
    setTimeout(function() { btn.textContent = "Save Draft"; btn.style.color = ""; }, 2000);
  }

  // FIX 2: Correct function signature (takes event, not lineNumber).
  //         Removed reads of cbEscalate/cbEmail/cbRegulatory — those
  //         checkboxes were commented out in the HTML.
  //         Success screen is shown here; submitCrmForm handles the API.
  function submitForm(e) {
    ...
    window.opener._crmFormExtras = {
        ...
        RecordID: document.getElementById("accountId").value || "",
        ...
        PriorityLevel: document.getElementById("priorityVal").value || "",

        ComplaintSummary: document.getElementById("descriptionBox").value  || "",
        ComplaintStatus:  document.getElementById("status").value           || "",

        Assignto: document.getElementById("assignTo").value || "",

        schedulecall: document.getElementById("callbackCheck").checked ? 1 : 0,

        ResolutionNotes: document.getElementById("resolutionNotes").value || "",

        // ✅ UPDATED (uses spinner time)
        CallbackDatetime: (
        ...
        Tags: Array.from(document.querySelectorAll("#tagsContainer .tag"))
                .map(function(t) { return t.childNodes[0].nodeValue.trim(); })
                .join(",")
    };

    // Call parent API function
    window.opener.submitCrmForm();
    ...
    var cbCheck = document.getElementById("callbackCheck");
    var cbDate  = document.getElementById("callbackDate").value;
    var cbEl    = document.getElementById("successCallback");

    if (cbCheck.checked && cbDate && timeValue) {
        var d = new Date(cbDate + "T" + timeValue);

        // ✅ 24-hour format
        var hours = String(d.getHours()).padStart(2, '0');
        var mins  = String(d.getMinutes()).padStart(2, '0');

        var fmt = d.toLocaleDateString("en-US", {
            weekday: "long",
            month: "long",
            day: "numeric"
        }) + " at " + hours + ":" + mins;
  ...
  function resetForm() {
    ...
    document.querySelectorAll(".priority-btn").forEach(function(b) { b.className = "priority-btn"; });
    document.querySelectorAll(".priority-btn")[1].classList.add("active-med");
    document.getElementById("callbackPanel").classList.remove("open");
    document.getElementById("callbackToggle").classList.remove("active");
    document.getElementById("callbackSummary").classList.remove("visible");
    document.querySelectorAll(".time-slot").forEach(function(s) { s.classList.remove("selected"); });
    // Restore pre-filled read-only fields after reset
    document.getElementById("accountId").value = "${safeRecord}";
    document.getElementById("phone").value     = "${safeClient}";
    document.getElementById("callbackDate").value = todayDate;
  }
<\/script>
</body>
...
function submitCrmForm() {
    try {
        var extras = window._crmFormExtras || {};

        if (!extras.RecordID) {
            alert("CRM form data missing.");
            return;
        }

        var formData = new URLSearchParams();

        formData.append("SipUsername",      extras.SipUsername      || "");
        formData.append("RecordID",         extras.RecordID         || "");
        formData.append("FirstName",        extras.FirstName        || "");
        formData.append("LastName",         extras.LastName         || "");
        formData.append("ID",               extras.ID               || "");
        formData.append("PhoneNo",          extras.PhoneNo          || "");
        formData.append("Channel",          extras.Channel          || "");
        formData.append("Category",         extras.Category         || "");
        formData.append("SubCategory",      extras.SubCategory      || "");
        formData.append("PriorityLevel",    extras.PriorityLevel    || "");
        formData.append("ComplaintSummary", extras.ComplaintSummary || "");
        formData.append("ComplaintStatus",  extras.ComplaintStatus  || "");
        formData.append("Assignto",         extras.Assignto         || "");
        ...
        fetch("https://192.168.5.60/SIPApi/api/v1/ivr/SavePopupForm", {
            method:  "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
        ...
        .then(function(text) {
            console.log("✅ CRM Updated:", text);
            delete window._crmFormExtras;
        })
        .catch(function(err) {
            console.error("❌ CRM Error:", err);
            alert("Error updating CRM: " + err.message);
        });
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    if(toggleBtn){
     ...
     document.addEventListener("kv-theme-changed", function(e){
        var h = document.documentElement;
        if(e.detail && e.detail.theme){
            h.setAttribute("data-theme", e.detail.theme);
        }
    });
...
function checkCallbacks() {
    ...
    fetch("https://192.168.5.60/SIPApi/api/v1/ivr/GetPendingCallbacks", {
        method: "POST",
        body: formData
    })
    .then(res => res.json())
    .then(data => {
        ...
        data.forEach(cb => {
            ...
            if (shownCallbacks.has(cb.RecordID)) return;

            shownCallbacks.add(cb.RecordID);

            showCallbackNotification(cb);
        });

    })
    .catch(err => console.error("Callback fetch error:", err));
}

checkCallbacks();
setInterval(checkCallbacks, 10000);

function showCallbackNotification(cb) {

    const safeName = `${cb.FirstName || ""} ${cb.LastName || ""}`.trim();
    const safeNum = cb.CallbackNum || "";
    const safeNote = cb.Callbacknotes || "";

    const html = `
    <div id="cb-${cb.RecordID}" style="
        position:fixed;
        bottom:20px;
        right:20px;
        width:280px;
    ...
    ">
        ...
        <div style="margin-top:10px;display:flex;gap:6px;">
            <button onclick="handleCallbackCall('${safeNum}','${cb.RecordID}','${safeName}')"
                style="flex:1;background:#238636;border:none;color:white;padding:6px;border-radius:6px;cursor:pointer;">
                📞 Call
            </button>

            <button onclick="handleRemindLater('${cb.RecordID}')"
                style="flex:1;background:#30363d;border:none;color:white;padding:6px;border-radius:6px;cursor:pointer;">
                ⏰ Later
            </button>
        </div>
    </div>
...
function handleCallbackCall(number, recordId, name) {

    if (!number) {
        alert("No number available");
        return;
    }
    ...
    if (!cleanNumber.startsWith("0091")) {
        ...
        if (cleanNumber.startsWith("91") && cleanNumber.length === 12) {
            cleanNumber = "00" + cleanNumber; 
        } 
        else if (cleanNumber.length === 10) {
            cleanNumber = "0091" + cleanNumber; 
        }
    }
    ...
    removeToast(recordId);
    updateCallbackStatus(recordId);
}


function handleRemindLater(recordId) {
    removeToast(recordId);

    const formData = new FormData();
    formData.append("recordId", recordId);

    fetch("https://192.168.5.60/SIPApi/api/v1/ivr/SnoozeCallback", {
        method: "POST",
        body: formData
    })
    .then(() => {
        console.log("⏰ Callback snoozed for 10 mins");
    })
    .catch(err => console.error("Snooze error:", err));

    // remove from shown list so it can reappear later
    shownCallbacks.delete(recordId);
}

function removeToast(recordId) {
    const el = document.getElementById(`cb-${recordId}`);
    if (el) el.remove();
}

function updateCallbackStatus(recordId) { 

    const formData = new FormData();
    formData.append("recordId", recordId);

    fetch("https://192.168.5.60/SIPApi/api/v1/ivr/UpdateCallbackStatus", {
        method: "POST",
        body: formData
    })
    .then(() => console.log("✅ Callback marked done"))
